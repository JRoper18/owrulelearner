...
internal class TweetyFolInstanceTest {
	...
	@Test
	fun testQuery() {
		assertEquals(TruthValue.TRUE, i1.query(parser.parseFormula("forall X: (isApple(X) => isRed(X))") as FolFormula))
		assertEquals(TruthValue.TRUE, i2.query(parser.parseFormula("forall X: (isApple(X) => isRed(X))") as FolFormula))
		assertEquals(TruthValue.UNKNOWN, i3.query(parser.parseFormula("forall X: (isApple(X) => isRed(X))") as FolFormula))
		assertEquals(TruthValue.UNKNOWN, i4.query(parser.parseFormula("forall X: (isApple(X) => isRed(X))") as FolFormula))
	...
	@Test
	fun testInfer(){
		assertEquals(mapOf(setOf<InferenceRule<FolFormula>>() to EvidenceInterval(1, 0, 1)), i1.infer("forall X: (isApple(X) => isRed(X))", setOf(), 1))
		assertEquals(mapOf(setOf<InferenceRule<FolFormula>>() to EvidenceInterval(2, 0, 2)), i1.infer("(isApple(X) => isRed(X))", setOf(), 1))

		//Now test it with given inference rules.
		//Imagine this rule is given 5 examples : 4 gala apples, and one granny smith.
		val r1 = InferenceRule(parser.parseFormula("isApple(X) => isRed(X)") as FolFormula, EvidenceInterval(4, 1, 5))
		//This rule is given the 4 gala apples.
		val r2 = InferenceRule(parser.parseFormula("isGala(X) => isRed(X)") as FolFormula, EvidenceInterval(4, 0, 4))
		//This rule could also be mined:
		val r3 = InferenceRule(parser.parseFormula("isApple(X) => isGala(X)") as FolFormula, EvidenceInterval.POSITIVE.scale(4.0))
		...
		assertEquals(mapOf(setOf<InferenceRule<FolFormula>>() to EvidenceInterval(0, 0, 1)), i5.infer("isRed(apple)", setOf(r1, r2), 0))
		//Now, try only using a single inference rule.
		assertEquals(mapOf(setOf<InferenceRule<FolFormula>>() to EvidenceInterval(0, 0, 1),
				setOf(r2) to EvidenceInterval(1, 0, 1),
				setOf(r1) to EvidenceInterval(0.8, 0.2, 1.0)
		), i5.infer("isRed(apple)", setOf(r1, r2), 1))
		//Using both inference rules should be the same, as either of them on their own produces a result.
		assertEquals(mapOf(setOf<InferenceRule<FolFormula>>() to EvidenceInterval(0, 0, 1),
				setOf(r2) to EvidenceInterval(1, 0, 1),
				setOf(r1) to EvidenceInterval(0.8, 0.2, 1.0)
		), i5.infer(parser.parseFormula("isRed(apple)") as FolFormula, setOf(r1, r2), 2))
		...
		), i6.infer(parser.parseFormula("isRed(apple)") as FolFormula, setOf(r2, r3), 2))
	}
}